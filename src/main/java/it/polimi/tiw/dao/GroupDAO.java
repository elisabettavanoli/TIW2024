...
public class GroupDAO {
	...
	public void createGroup(Group g, List<String> users, User creator) throws SQLException {
	...
	public ArrayList<User> getParticipants(int id) throws SQLException {
		...
		String query = "SELECT u.*" + "FROM participations p " + "JOIN user u ON p.user = u.username "
	...
	public void removeParticipant(String participantUsername, int groupId) throws SQLException {
		String query = "DELETE FROM participations WHERE user = ? AND gruppo = ? AND status = 'invited'";
		try (PreparedStatement pstatement = con.prepareStatement(query)) {
			pstatement.setString(1, participantUsername); // Imposta lo username del partecipante
